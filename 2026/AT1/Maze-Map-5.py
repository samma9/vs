...
# map is list of strings, each string is a row of the map. 
...
def pos_update(command, map_position):
    if command == "forward":
        map_position[0] -= 1
...
def game_loop():
    ...
    while True:
        ...
        if command in ("forward", "backward", "left", "right"):
            map_position = pos_update(command, map_position)
